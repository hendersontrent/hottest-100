// Builds logit regression model with interaction   //
...
transformed data {
  ...
  vector[N] iter;   // Instantiate interaction
  iter = x1 .* x3; // Interaction term
...
model {
  ...
  y ~ bernoulli_logit(alpha + beta[1] * x1 + beta[2] * x2 + beta[3] * x3 + beta[4] * x4 + beta[5] * iter);
...
generated quantities {
  ...
  for (n in 1:N) {
    y_hat[n] = inv_logit(alpha + beta[1] * x1[n] + beta[2] * x2[n] + beta[3] * x3[n] + beta[4] * x4[n] + beta[5] * iter[n]);
  }
}